...
def generate_prompt(question, candidate_answers, prompt_type, N,
    ...
    if top_k: # task 5
        ...
        # similarity = torch.matmul(question_embeddings, demonstration_embeddings.T) # [1, n_demo]
...
def get_model(
    ...
    assert os.path.exists(model_dir), (
        f"Model directory {model_dir} does not exist. Please check the path."
    )
...
def main():
    ...
    tokenizer, model = get_model(model_dir=args.model)
